...
class Enhet  {             //  = TELT også !!!
  ...
  public:
    Enhet()  {  cout << "'Enhet' MAA opprettes med feltnummer!\n";  }
    ...
    virtual ~Enhet()  {  cout << "'Enhet'-objekt dor.\n";  }
...
class Bobil : public Enhet {
  ...
  public:
    Bobil() { cout << "'Bobil' MAA opprettes med feltnummer!\n"; }

    Bobil(int nr) : Enhet(nr)      //  Setter initielle data:
                    {  strom = false;   type = bobil;   grunnpris = BIL;  }

    virtual ~Bobil()  {  cout << "'Bobil'-objekt dor.\n";  }
...
void Bobil::lesData() {
    Enhet::lesData();
    strom = (lesChar("\tBruke strom (j/N)") == 'J');
}
...
void nyGjest() {                  //  Registerer en ny gjest i lista.
    ...
    if (feltNr) {                              //  '0' (null) er IKKE skrevet:
        if (!gFelt[feltNr-1])  {               //  Feltet er ledig:
         ...
         cout << "\n\tFeltet er allerede opptatt, prov med et annet!";
    } else
 ...
 */
void plassensStatus() {
    cout << "\n\nStatus for ALLE feltene paa HELE campingplassen:\n";
...
void slettGjest() {
    ...
    if (gFelt[feltNr-1]) {                      //  Feltet er korrekt i bruk:
        ...
        cout << "\n\tIngen registrert paa dette feltet. Feil feltnummer?\n";
}
