  ...
  if (aar >= 1900 && aar <= 2027) {              //  Årgang er i OK intervall:
     ...
     if (nr != -1) {                             //  Årgangen finnes:
        if (ant > 0 || -ant <= antall[nr]) {     //  Får nye eller nok igjen:
           ...
           cout << "\n\tAntall paa lager av aargang "      //  Skriver nytt
                << aar << ":  " << antall[nr] << "\n\n";   //    antall.
...
void Vin::skrivData()  const  {
    cout << '\t' << navn << ",  #aarganger: " << aargang.size() << "\n\t\t";
...
void skrivMeny()  {
   cout << "\nFolgende kommandoer er tilgjengelig:\n"
        ...
        << "\tP - ny Produsent\n"
        << "\tV - ny Vin\n"
        << "\tE <nr>  <(nytt) aar>  <ant.flasker inn(+)/ut(-)>\n\t      "
        << "- Endre vin ( (ny) aargang og/eller antall (selge/etterfylle) )\n"
        << "\tQ - Quit / avslutt\n";
}
