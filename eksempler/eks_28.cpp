...
void Lege::fjernResepter(const int dato) {
    ...
    if (!resepter.empty()) {            //  Legen HAR resepter:
        ...
        cout << "\t\t#resepter etterpaa:  " << resepter.size() << '\n';
    }
}
...
void fjernResepter() {                   //  De FØR("!!!!) datoen fjernes:
    int fDato = lesInt("\tFjerne resepter FOR dato", 200101, 253112);
    cout << '\n';
...
//                                     (val.second)->skrivHovedData();  } );
        ...
        cout << "\n\tIngen leger (og resepter) registrert forelopig!\n\n";
}
