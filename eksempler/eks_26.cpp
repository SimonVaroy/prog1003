...
int main()  {
    ...
    while (kommando != 'Q') {
        switch (kommando) {
          ...
          case 'S':    slette();                         break;
          case '1':    skrivKo("heisko", gHeisKo);       break;
...
void Kjorer::endreData() {
    cout << "\n\tEndre datamedlemmer:  IKKE IMPLEMENTERT ENNAA\n\n";
...
void Kjorer::skrivDetaljer() const {
    ...
    cout << "\n\tAltsaa: " << antG << " gang(er) i GRONN, og "
                           << antR << " gang(er) i ROD loype.\n\n";
}
...
void Snowboarder::lesData() {
    ...
    morkHjelm  = (lesChar("\tMork hjelm?  (J/n)") != 'N');
}
...
void detaljerEllerEndre(const Operasjon operasjon) {
    ...
    if (!gGronn.empty() || !gRod.empty()) {    //  Kjører(e) i MINST en løype:
        ...
        if (kjorer) {                          //  Kjøreren funnet:
          ...
          cout << "\n\tUkjent nummer i begge loypene!\n\n";
    } else
        cout << "\n\tBegge loypene er tomme!\n\n";
}
...
void nyKjorer() {
    ...
    cout << "\tNy kjorer har nr." << ++gSisteNummer << ":\n";
 ...
 */
void skrivGronn() {
    cout << "\nFolgende " << gGronn.size() << " kjorere er i GRONN løype:\n";
 ...
 */
void skrivKo(const string & s, queue <Kjorer*> ko) {
    cout << "\nFolgende " << ko.size() << " kjorere er i " << s << ":\n";
...
void skrivMeny() {
    cout << "\nFolgende kommandoer er tilgjengelig:\n"
         << "\tK - inn i heisKo\n"
         << "\tH - fra heisko til Heisen\n"
         << "\tG - fra heisen til Gronn loype\n"
         << "\tR - fra heisen til Rod loype\n\n"
         << "\tN - Ny kjorer\n"
         << "\tD - Detaljer om EN kjorer  (KUN mulig naar er i en loype)\n"
         << "\tE - Endre en kjorer        (KUN mulig naar er i en loype)\n"
         << "\tS - Slett/fjern en kjorer  (KUN mulig naar er i en loype)\n\n"
         << "\t1 - skriv HEISKO\n"
         << "\t2 - skriv HEISEN\n"
         << "\t3 - skriv GRONN loype\n"
         << "\t4 - skriv ROD loype\n\n"
...
void slettAlleData() {
    ...
    cout << "\tAntall elementer i HEISKOEN:     " << gHeisKo.size() << '\n';
...
//      gGronn.erase(it);                     //  Sletter PARET key og PEKER.
    ...
    cout << "\tAntall elementer i GRONN LOYPE:  " << gGronn.size() << '\n';
    ...
    cout << "\tAntall elementer i ROD LOYPE:    " << gRod.size() << '\n';
}
...
void tilHeisen() {
  if (!gHeisKo.empty()) {                   //  Noen i heiskøen:
    ...
    cout << "\n\tflyttes fra heiskoen og inn (nederst/bakerst) i heisen.\n\n";
      gHeis.push(gHeisKo.front());          //  KOPIERER pekeren til 'gHeis'.
      gHeisKo.pop();                        //  Sletter 1.pekeren i 'gHeisKo'.
  }  else
      cout << "\n\tHeiskoen er tom, saa ingen aa sette inn i heisen!\n\n";
}
...
void tilHeisKo()  {
    ...
    if (!gGronn.empty()  ||  !gRod.empty()) {     //  Kjører i minst EN løype:
        cout << "\tFlytte fra en loype til heisko:\n";
        ...
        if (kjorer) {                         //  Kjøreren funnet:
            ...
            if (loype == 'G') {               //  Funnet i GRØNN løype:
               cout << "\n\tflyttes FRA GRONN loype og inn i heiskoen.\n\n";
               gHeisKo.push(kjorer);          //  Legger inn Kjorer-PEKER.
               gGronn.erase(nr);              //  Sletter PARET key og PEKER.
           } else if (loype == 'R') {         //  Funnet i RØD løype:
               cout << "\n\tflyttes FRA ROD loype og inn i heiskoen.\n\n";
               gHeisKo.push(kjorer);          //  Legger inn Kjorer-PEKER.
...
//                             ((gHeis.front())->hentID(), gHeis.front()));
    ...
    cout << "\n\tHeisen er tom, saa ingen aa sette inn i noen loype!\n\n";
}
